...
void getUserInput(char *fieldName, char *fieldValue)
{
	...
	while(fread(fieldValue, sizeof(fieldValue), 1, fpData))
	{
		...
		if(strcmp(fieldValue, recordId) == 0)
		{
			...
			if(strcmp(recordStatus, "A") == 0)
			{
				...
				// printf("position - %d", recordPosition);
			}
			break;
		}
	}
...
void getNewUpdateValue(char* fieldName, char* newFieldValue)
{
	printf("Enter %s ", fieldName);
	scanf("%s", newFieldValue);
	// printf("status - %s", newFieldValue);
...
void searchRecord()
{
	...
	getUserInput(fieldNames[1], recordId);
...
void updateRecord()
{
	...
	if(recordPosition != -1)
	{
		...
		fread(fieldValue, FIELD_VALUE_LENGTH, 1, fpData);
		getNewUpdateValue(fieldNames[fieldsCount - 1], newFieldValue);
		fseek(fpData, -sizeof(fieldValue), SEEK_CUR);
...
void createRecord()
{
	...
	for(counter = 0; counter < fieldsCount; counter++)
	{
		...
		{
			getUserInput(fieldNames[counter], fieldValue);
			fwrite(fieldValue, sizeof(fieldValue), 1, fpData);
